...
class StoreClientRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'categorie' => ['sometimes', 'required', 'string', 'in:gold,bronze,silver'],
            'max_montant' => ['required_if:categorie,silver', 'integer'],
            'user' => ['sometimes', 'array'],
            'user.nom' => ['required_with:user', 'string'],
            'user.prenom' => ['required_with:user', 'string'],
            'user.photo' => ['required_with:user', 'image'],
            'user.email' => ['required_with:user', 'string', 'email', 'max:255', 'unique:users,email'],
            // 'user.password' => ['required_with:user', 'string', 'confirmed', new PasswordRule()],
        ];
    }
